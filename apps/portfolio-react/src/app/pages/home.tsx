...
export function Home() {
  ...
  const loadCollection = async (target: EnvKey) => {
    try {
      const target_state =
        target === 'emulator' ? setEmulatorState : setLiveState;
      target_state((prev) => ({ ...prev, isLoading: true }));

      const docs = await firestoreListCollection<TestDoc>(
        PROJECT_KEY,
        target,
        COLLECTION_PATH
      );
                  ...
                  return (
                    <div>
                      ...
                      {envState.savingError && (
                        <p style={{ color: 'red' }}>
                          Save error: {envState.savingError}
                        </p>
                      )}
                      ...
                      {vm.data.map((item) => (
                        <div key={item.id}>
                          ...
                          <div>
                            <button onClick={() => quickEdit(env, item)}>
                              Quick edit
                            </button>
                            <button onClick={() => deleteDoc(env, item)}>
                              Delete
                            </button>
                          </div>
                        </div>
